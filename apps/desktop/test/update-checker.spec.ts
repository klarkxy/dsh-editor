...
describe('selectAsset', () => {
  const assets = [
    { name: 'DSH-Editor-Setup-0.3.0-win-x64.exe', url: 'u-setup', size: 1 },
    { name: 'DSH-Editor-0.3.0-win-x64.exe', url: 'u-portable', size: 2 },
    { name: 'DSH-Editor-0.3.0-win-x64.zip', url: 'u-zip', size: 6 },
    { name: 'DSH-Editor-0.3.0-mac-arm64.dmg', url: 'u-dmg', size: 3 },
    { name: 'DSH-Editor-0.3.0-mac-arm64.zip', url: 'u-maczip', size: 4 },
  ...
  it('picks the portable exe (not the setup exe or zip) for portable Windows', () => {
  ...
  it('returns null when the portable build is only published as a zip', () => {
    expect(selectAsset([{ name: 'DSH-Editor-0.3.0-win-x64.zip', url: 'u', size: 1 }], 'win32', true)).toBeNull()
  })
  it('returns null when nothing matches', () => {
    expect(selectAsset(assets, 'linux', false)).toBeNull()
    expect(selectAsset([], 'win32', false)).toBeNull()
    expect(selectAsset([{ name: 'DSH-Editor-0.3.0-win-x64.zip', url: 'u', size: 1 }], 'win32', false)).toBeNull()
  })
})
...
describe('buildDownloadCandidates', () => {
  ...
  it('rejects non-official GitHub release URLs instead of downloading them directly', () => {
    expect(() => buildDownloadCandidates('https://example.com/file.zip')).toThrow('官方 GitHub')
    expect(() => buildDownloadCandidates('https://github.com/evil/other/releases/download/v1/x.exe')).toThrow('官方 GitHub')
  })
})
