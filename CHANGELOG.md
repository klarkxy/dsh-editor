...
本文件按版本记录变化，尚未发布的进展集中在「未发布」段；安装、升级和回滚步骤以 [使用者指南](docs/user-guide.md) 为准。
...
- 新对话先选择写作模式：通用写作（默认）、小说创作、文章与自媒体、技术文档。后三种可在设置「插件 → 写作模式」里关闭。旧版 `dsh-editor` 会话会看到迁移提示，仍可继续打开。
- 文稿校对从侧栏恢复：`Ctrl+Shift+L` / 命令面板「校对当前文档」；「校对全部文档」扫全部可见 Markdown / TXT。
- 章节规划改到落笔前，正文页不再出现规划入口。
- 顶栏改为图标，对话以 overlay 打开。

### 写作界面

- 搭档栏输入区显示当前对话模式；思考强度紧挨模型，档位为 none / low / medium / high / xhigh / max。
- 作者工作台与导航精简；文件提案卡重新堆叠。
- 章纲与章末小结拆分为独立编辑，并支持大纲、章纲与章末小结提案。
- 启动页增加加载动画；启动失败可通过 preload 重试。
- 历史版本列出相对上一版的文件增减；文件栏与搜索继续隐藏辅助文件。
- 宽屏栏宽可拖动；文件树支持键盘导航，并补齐加载与空状态。

### 活动反馈

- 建立统一的异步活动反馈：编辑工具与独立插件共用同一套工作中 / 完成状态。

### 前置规划

- 章节规划前置到正文之前，正文页不再出现规划入口。
...
- 写作功能开关只显示开或关，不再出现「部分启用」。
- 市场安装改用编译后的 npm 产物，卸载重启安全。
- 市场 GitHub 链接改在系统浏览器打开。
- 第一方写作模式随桌面打包，可按作者开关。

### 设置

- 「关于」与插件设置由外壳托管，支持规划提案预览。
- 助手分区可保存跨作品作者约定。
...
- 界面文案更贴近作者：去掉「契约」「宿主」「智能体」等技术词，错误与提示改说人话，四个写作模式的描述改为完整句式。
- 稿纸即时预览会收起链接的括号和目标地址，不再把 `%20` 编码的路径摊在正文里。
- 禁止终端等编码工具只对四个写作模式（及旧版 `dsh-editor`）生效；官方 Agent 模式不再被拦截。
- 失效作品会话自动重连。
- 搭档提案带版本基线：文件在提案生成后发生变化时拒绝覆盖。

### 文档

- 文档收成使用者指南、产品原则、架构决策与包级 README；架构图增加提案版本门禁。

### 内部

- 界面改用 Radix Themes，去掉手写纸 / 墨控件皮肤。
- 精简测试：日常 `pnpm test` 只跑 Vitest；桌面验收保留 Electron 与核心闭环；交付再跑插件矩阵、缺包与便携包。删掉未进 CI 的 live / 截图 / 人工审计 e2e，校对引擎用例回到 `dsh-proofread`。
- 优化 dev 构建链接、清理一次性验证脚本。

## 0.2.0 - 2026-09-12

### 升级说明

- 桌面校对暂时停用：顶栏、正文菜单、命令面板和快捷键不再提供校对入口。已有的作品、校对名单和结果文件保留。
- 知乎资料移到「设置 → 知乎资料」。可在这里配置凭据、查看用量和管理知识库，搭档仍可调用知乎工具。
- 文件栏与作者全文搜索隐藏 `AGENTS.md` 等辅助文件；原文件保留，作者搜索替换不会修改这些文件。

### 写作界面

- 整理纸 / 墨双主题、文件栏、设置弹窗和菜单。版本保存与历史查看收进文件栏菜单，长作品名不会再挤到顶栏按钮。
- 修复写作设置无法滚动到底的问题，统一滑块样式；作者约定可以在底部保存。
- 用量页采用 Apache ECharts 柱状图，显示日期刻度、分模型用量、悬浮明细与精确数据。空日期按零显示，长模型名可以换行查看。
- 搭档对话支持展开阅读；模型选择显示来源，推理强度单独设置。选段改写的应用入口和执行结果更清楚。
- 正文右键与更多菜单共用编辑操作，支持原生纯文本剪贴板、撤销 / 重做及选区改写。输入法组字、文档切换和异步操作期间校验编辑目标，避免把旧结果写进其他章节。
- 人物卡、世界书、作品概览及记忆维护面板跟随双主题和键盘交互；记忆确认、撤销后的条目状态保持展开。

### 插件与数据保护

- 内置插件按作者用途分组，一个开关控制同组入口。修复 Windows 保存插件开关时的重命名失败与部分生效问题。
- 插件状态持久化失败时恢复原状态；读取或覆盖用户自定义配置受阻时停止变更，保留配置和已安装文件。
- 图表提示将模型名称作为普通文字显示，避免将名称中的标记解释为 HTML。

### 文档与发布检查

- 更新使用指南、开发与插件合同、包级 README 和架构图，区分 0.2.0 当前说明与历史验证记录。
- 发布流程先构建跨包声明，再做类型和测试检查；最终下载包单独验证版本、校验和、启动保存与退出。

## 0.1.9 - 2026-09-11

### 升级说明

- 项目规则统一写在作品根目录 `AGENTS.md`。人物卡、世界书、总纲与索引改为按需读取；旧触发词 / 启用 / 优先级不再控制自动注入，稿纸触发表单已移除。原文件可继续保留，设定详情仍支持分类、标签和摘要。
...
- 世界书字段检查改走卡片详情；全文替换等当前编辑保存完成后再执行；导出验收核对 Markdown、DOCX、EPUB 的实际正文。
- 发布前校验 tag 与应用版本号，拒绝发布页和安装包版本不一致的构建。
- 修复全新检出的构建 / 类型检查顺序，并让插件路径测试使用当前系统的合法文件 URL。0.1.8 在 CI 阶段未通过，未发布安装包；本版包含这些修复。
...
未通过 CI 阶段的检查，未发布安装包；变化并入 0.1.9。
...
- 工具调用失败更醒目：失败行默认展开，带 ⚠ 警示与红色标识，并直接给出中文原因；被工具守卫拦截时明确说明「只允许查找、阅读作品资料与可预览提案」，不再只显示一行含糊的「这项操作没有执行」。
- 助手现在可以向你提问：`ask_user_question` 经守卫限量放行（每次 1–4 个简短问题，可附选项），动笔前先请你拍板方向，不再因调用被禁而空转一轮。
- 新增 `.dsh-editor/scratch/` 临时工作区：助手可用 `novel_scratch_write/read/list` 在其中自由读写工作草稿（限 .md/.txt、单文件 2 万字符、最多 20 个文件），路径软禁在目录内；草稿不进上下文信封、不进作者自管的 git（自动维护 `.gitignore`），界面也不显示这些操作。
- 写作会话改用专属 `dsh-editor` agent preset：只挂载查找、阅读、提问与上下文压缩，shell、write/edit、todo、web、subagent 等编码工具不再出现在助手的工具目录里，从源头上避免「看得见却用不了」的误调用；preset 随桌面应用启动自动部署，与 profile 同一套防覆盖规则。
- novel-kernel 知识卡新增 `deai`（去 AI 腔）主题：蒸馏自 sepia 0.8.0 的分层审查协议、HC3-Chinese 中文实测信号、StoryScope 叙事层指纹、删换优先的修复手法与白名单，知识资产版本升至 2；打包校验同步覆盖新卡。

### 提案与写入

- edit 提案允许 `oldText` 为空以填充仍是空白的文件；create 提案允许覆盖空白占位文件（如提前建好标题的章节）。非空文件依旧拒绝，正文不会被静默覆盖。

### 更新与打包

- 一键下载更新：「关于」对话框内直接下载新版，按自定义镜像（`DSH_UPDATE_MIRRORS`）→ 内置镜像 → 直连的顺序尝试，下载完成按发布页附带的 `sha256sums.txt` 校验完整性；Windows 安装版「退出并安装」由向导覆盖安装，便携版「重启并替换」自动完成 EXE 替换并重启，macOS 下载后打开所在文件夹手动替换。
- release 附件新增 `sha256sums.txt`，供应用内下载校验。
...
- 桌面 e2e 不再继承 `ELECTRON_RUN_AS_NODE`（VS Code 终端会注入该变量导致 Electron 秒退）；Electron dist 缺失或残缺时改用 `@electron/get` 下载 + 系统解压器重建（electron 自带 install.js 依赖的 extract-zip@2 解大 zip 会静默挂死）。
- 本地 DSH 服务随机到 Chromium 禁用端口（如 6665，页面报 ERR_UNSAFE_PORT）时自动换端口重启，最多重试 5 次。
