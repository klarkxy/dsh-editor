...
- 外壳顶栏改为图标化，对话改为 overlay 形式，工具错误提示更安静。
...
- 建立统一的异步活动反馈体系：共享反馈原语、异步工作流动画，覆盖编辑工具与独立插件的活动状态，原语沉淀在 `packages/dsh-editor-shell/src/client/ui/activity.tsx`。
...
- 禁止终端等编码工具只对四个写作 Preset（及旧版 `dsh-editor`）生效；官方 Agent 模式不再被拦截。
- 失效作品会话自动重连。
