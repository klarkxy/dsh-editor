...
module.exports = function(file, api, options) {
  ...
  const canLiteralBePropString = node => {
    ...
    return raw.indexOf('\\') === -1 && node.value.indexOf('"') === -1 && node.value.indexOf('\\') === -1;
  };
  ...
  const convertExpressionToJSXAttributes = expression => {
    ...
    if (isReactSpread || isObjectAssign) {
      ...
      const attributes = expression.properties.map(property => {
        if (property.type === 'SpreadProperty' || property.type === 'SpreadElement') {
          ...
          let value;
          if (
            (propertyValueType === 'Literal' || propertyValueType === 'StringLiteral') &&
          ...
          let jsxIdentifier;
          if (property.key.type === 'Literal' || property.key.type === 'StringLiteral') {
    ...
    } else if ((expression.type === 'Literal' || expression.type === 'NullLiteral') && expression.value === null) {
      return {
        attributes: [],
        extraComments: expression.comments || []
      };
    } else {
      /* local patch: TS `as` casts and other exotic prop expressions map to
         a spread attribute, preserving the original semantics. */
      return {
        attributes: [j.jsxSpreadAttribute(expression)],
        extraComments: []
      };
  ...
  const isCapitalizationInvalid = node =>
    ((node.type === 'Literal' || node.type === 'StringLiteral') && !/^[a-z]/.test(node.value)) ||
  ...
  /* local patch: hasReact predates StringLiteral AST nodes and always
     returns false with the tsx parser; every target file imports react. */
  if (options['explicit-require'] === false || true || ReactUtils.hasReact(root)) {
