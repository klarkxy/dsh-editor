...
- 界面语言默认简体中文；English 与中文 key 一一对应（含右侧 DSH 对话的 `chat.*`），由 `i18n.spec.ts` 锁定，切换后立即生效。
...
| --- | --- |
| `> 1040px` | 三栏 |
| `≤ 1040px` | 搭档栏改为覆盖稿纸的抽屉，点顶栏「搭档」打开；不再占第三列或拦截稿纸点击。作品名菜单仍留在顶栏。 |
| `≤ 760px` | 只留稿纸，搭档仍用同一抽屉；文件栏开关不可用，作品名菜单与搭档开关仍可见。 |
