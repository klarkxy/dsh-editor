...
发布使用官方 `npm publish`。最终 tarball 再次校验内容 hash，并在上传后回读确切版本的 `dist.integrity` 与 `dshRelease.contentHash`。注册表读取使用独立查询参数避开旧缓存，确认重试逐步延长等待，累计退避最多三分钟，以覆盖上传成功后公开元数据延迟可见的情况。上传响应丢失时会先对账，不立即递增另一个版本。无依赖关系的包互不阻塞：只回写已确认成功包的版本；失败包及其下游保留失败记录。成功版本和内容 hash 回写各自 `package.json`，不会创建桌面发布标签。
