...
最近的界面调整遵守这些边界：桌面校对暂停，知乎配置移入设置；辅助配置文件从作者文件树和搜索中隐藏，原文件与搭档读取能力保留。
...
- 作品是普通文件夹。新建预建空的 `正文/`、`大纲/`、`人物卡/`、`世界书/` 四类目录，但不生成故事模板。不预写向导，不用隐藏 JSON 当作作品结构。

## 先提案，再由作者确认

正文修改、未经确认的设定和事实冲突保持为可见候选：应用或忽略。落笔前的初始人物卡、世界书和大纲/章纲都必须先形成可见提案；大纲与章纲统一保存为 `大纲/` 下的 Markdown，不写入正文隐藏元数据。后续协作中自然发现、来源可靠且无冲突的人物或世界资料，可以通过有来源、版本检查和可撤回记录的维护工具自动保存；修订已有内容仍先确认。已应用的文本修改在提案卡仍在、且目标文件未再变化时可撤销。新建文件不提供自动删除式撤销；不需要的文件走归档。
