...
公开插件 `dsh-grill` 仍可在普通 DSH Web profile 里提供 planning / drafting / review / first-reader 提示和 `scaffold_novel`；桌面 profile 不加载它。桌面写作会话走专属 `dsh-editor` agent preset，不挂载官方编码工具目录。
