...
2. ~~章节状态与全书进度已经落地~~（固定三状态后因缺少实际消费面已整体移除；概览的字数/章节扫描保留，后续观察 2,000 文件/100 MB 上限内的真实扫描性能）。
...
本轮曾落地这一项，后因状态没有实际消费面（既不参与上下文编译也不做流程门禁，仅是展示标签与 agent 可读元数据）整体移除：`.dsh-editor/chapter-status.json`、`chapter.statusSet`、`novel_set_chapter_status` 工具、徽标与三态下拉全部拆除；`project.overview` 的字数与章节摘要保留。
