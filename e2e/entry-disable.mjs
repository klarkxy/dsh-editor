...
try{
 ...
 const absent=await page.request.post(new URL('/proofread/text.check',started.url).href,{data:{type:'client-request',rpcId:'absent',method:'text.check',payload:{text:'以经'}}});const fallback=await page.request.post(new URL('/no-such-plugin/text.check',started.url).href);if(absent.status()!==fallback.status()||![404,405].includes(absent.status()))throw new Error('disabled proofread RPC did not match absent route');
 ...
 await page.getByRole('button',{name:'设置',exact:true}).click();await page.locator('.settings-nav').getByRole('tab',{name:'知乎资料',exact:true}).click();await page.getByTestId('zhihu-settings-embed').waitFor();await page.getByRole('button',{name:'关闭设置',exact:true}).click();await page.locator('.settings-dialog').waitFor({state:'hidden'});report.checks.push('disabled proofread contributes neither Host route nor Client; Zhihu settings still work');
 ...
 const checked=await page.request.post(new URL('/dsh-editor-workbench/proofread.scan',started.url).href,{data:{type:'client-request',rpcId:'engine',method:'proofread.scan',payload:{sessionId,scope:'document',path:'正文/001.md'}}}).then(r=>r.json());if(!checked.result?.ok||!JSON.stringify(checked.result.value).includes('已经'))throw new Error('retained workbench engine failed '+JSON.stringify(checked));
