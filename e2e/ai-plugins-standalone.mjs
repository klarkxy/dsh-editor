...
try {
  ...
  const methodCard = review.locator('.si-card').filter({ has: page.getByText('局部修改先读后核对', { exact: true }) })
  ...
  await secondReview.getByText('核对资料来源', { exact: true }).waitFor()
  ...
  await persistedReview.getByText('局部修改先读后核对', { exact: true }).waitFor()
  await persistedReview.getByText('核对资料来源', { exact: true }).waitFor()
