...
try {
  ...
  await right(0)
  assert.equal(await page.getByTestId('editor-menu-chapter-meta').count(), 0, 'chapter outline is absent from the manuscript menu')
  assert.equal(await page.getByTestId('editor-menu-chapter-summary').count(), 0, 'chapter summary is absent from the manuscript menu')
  assert.equal(await page.getByTestId('editor-menu-wrap-up').count(), 0, 'wrap-up is absent from the manuscript menu')
  await page.keyboard.press('Escape')
  await page.locator('.editor-action-menu').first().waitFor({ state: 'detached' })
  mark('chapter meta and wrap-up have no manuscript menu action')
  await select(0, 2)
  await right(1)
  await page.getByTestId('editor-menu-rewrite').click()
  const custom = page.getByRole('dialog', { name: '自定义改写' })
  await custom.waitFor()
  assert(await custom.getByLabel('输入改写要求').evaluate(el => el === document.activeElement), 'custom dialog owns focus')
  await page.keyboard.press('Escape')
  await page.locator('.editor-action-menu').first().waitFor({ state: 'detached' })
  ...
  await overflow()
  assert.equal(await page.getByTestId('editor-menu-chapter-meta').count(), 0)
  assert.equal(await page.getByTestId('editor-menu-chapter-summary').count(), 0)
  assert.equal(await page.getByTestId('editor-menu-wrap-up').count(), 0)
  await page.keyboard.press('Escape')
  await page.locator('.editor-action-menu').first().waitFor({ state: 'detached' })
  assert.equal(await text(), edited)
  mark('chapter meta and wrap-up entries stay absent while manuscript remains untouched')
