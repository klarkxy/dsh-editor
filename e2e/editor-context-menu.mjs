...
try {
  ...
  const meta = page.getByRole('dialog', { name: '章纲', exact: true })
  await meta.waitFor()
  ...
  const chapterForm = page.getByRole('dialog', { name: '章纲', exact: true })
  ...
  await command('chapter-summary')
  const summaryForm = page.getByRole('dialog', { name: '章末小结', exact: true })
  await summaryForm.getByLabel('此刻', { exact: true }).fill('等待靠岸')
  await summaryForm.getByRole('button', { name: '写入', exact: true }).click()
  await summaryForm.waitFor({ state: 'detached' })
  assert.equal(await text(), edited)
  // Metadata dialogs await the real save; the explicit Save menu is disabled once clean.
  ...
  mark('separate chapter plan and summary save through editor while preserving visible manuscript')
