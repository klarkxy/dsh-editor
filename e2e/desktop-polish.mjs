...
try {
  ...
  const sideSearch = page.getByRole('searchbox', { name: '搜索作品文字' })
  await sideSearch.fill('隔离测试辅助文件'); await sideSearch.press('Enter')
  ...
  if (after.scroll > after.height) assert(after.top>before.top, 'writing tab must really scroll with the wheel')
  assert((await page.locator('.paper-typography [role=slider]').count()) >= 1, 'writing typography must expose a Themes slider')
  await shot('03-writing-bottom')
  report.metrics.writingScroll={before,after}
  mark('writing settings expose paper controls')
  await closeSettings()
  await openSettings('助手')
  const savePrefs = page.getByRole('button',{name:'保存作者约定',exact:true})
  assert(await within(savePrefs), 'author save control is clipped')
  await page.getByRole('textbox',{name:'跨作品作者约定',exact:true}).fill('保持第三人称限知，允许留白。')
  await savePrefs.click()
  mark('assistant settings save author preferences')
  await closeSettings()
  await openSettings('助手')
  assert.equal(await page.getByRole('textbox',{name:'跨作品作者约定',exact:true}).inputValue(),'保持第三人称限知，允许留白。')
  mark('assistant settings persist author preferences')
  await closeSettings()
  ...
  for (const theme of ['light', 'dark']) {
    ...
    for (const kind of ['empty', 'single', 'mixed']) {
      ...
      if (kind === 'empty') {
        ...
        assert(tooltipText.includes('51K'), 'tooltip should expose the compact MiniMax total')
        if(kind === 'mixed') assert(tooltipText.includes('77K'), 'tooltip should expose compact combined daily total')
        await shot('06-usage-' + theme + '-' + kind)
        const log = page.locator('.usage-log')
        await log.waitFor()
        const logText = await log.innerText()
        assert(logText.includes('51K'), 'usage log should list the latest MiniMax request')
        if(kind === 'mixed') assert(logText.includes('26K'), 'usage log should list the other model request')
        const overflow = await page.locator('.settings-pages').evaluate(el => el.scrollWidth - el.clientWidth)
        if(overflow>1)report.errors.push('usage log creates horizontal overflow in settings: '+overflow)
