...
import { validateEntryName } from './lifecycle.ts'
import type { OverviewAccess } from './overview.ts'
import { createDirectory } from './project.ts'
...
async function ensureCardRoot(access: OverviewAccess, root: typeof CHARACTER_ROOT | typeof WORLDBOOK_ROOT): Promise<void> {
  try {
    ...
    if (error instanceof FileOpError && error.code === 'NOT_FOUND') {
      await createDirectory({ root: access.path, mode: access.mode, relative: root, signal: access.files.signal })
      return
    }
    throw error
  }
