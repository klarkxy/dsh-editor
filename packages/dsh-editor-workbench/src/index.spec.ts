...
describe('proposal dispatch endpoints', () => {
  ...
  it('applies a V2 create when basis is unchanged and reports STALE after the source changes', async () => {
    const root = await projectRoot()
    try {
      await fs.writeFile(path.join(root, '正文', '001.md'), '# 第一章\n', 'utf8')
      const { host } = fixture()
      ...
      const handler = rpcHandler(host)
      const signal = new AbortController().signal
      const source = await host.fs.stat(await host.fs.resolve('正文/001.md', { cwd: root }))
      const proposal = {
        marker: 'dsh-editor.proposal',
        version: 2,
        kind: 'create',
        ...
        path: '世界书/港口.md',
        text: '# 港口\n',
        basis: [{ path: '正文/001.md', version: String(source!.version), label: '正文' }],
      }
      const preview = await handler('proposal.prepare', { sessionId: 'session-1', proposal }, signal)
      expect(preview).toMatchObject({ ok: true, value: { create: { applicable: true, missingDirectories: ['世界书'] } } })
      expect(await handler('proposal.apply', { sessionId: 'session-1', proposal, expectedVersions: { [proposal.path]: '' } }, signal))
        .toMatchObject({ ok: true, value: { path: proposal.path, operation: 'create' } })
      expect(await fs.readFile(path.join(root, '世界书', '港口.md'), 'utf8')).toBe('# 港口\n')

      const staleProposal = {
        ...proposal,
        path: '世界书/海关.md',
        basis: [{ path: '正文/001.md', version: String(source!.version) }],
      }
      await fs.writeFile(path.join(root, '正文', '001.md'), '# 第一章\n已改\n', 'utf8')
      const stalePrepare = await handler('proposal.prepare', { sessionId: 'session-1', proposal: staleProposal }, signal)
      expect(stalePrepare).toMatchObject({ ok: false, error: { details: { reason: 'STALE' } } })
      const staleApply = await handler('proposal.apply', { sessionId: 'session-1', proposal: staleProposal, expectedVersions: { [staleProposal.path]: '' } }, signal)
      expect(staleApply).toMatchObject({ ok: false, error: { details: { reason: 'STALE' } } })
      await expect(fs.stat(path.join(root, '世界书', '海关.md'))).rejects.toMatchObject({ code: 'ENOENT' })
    } finally {
      await fs.rm(root, { recursive: true, force: true })
    }
  })

  it('applies a V2 create to a visible .txt file, prepares a TXT split, and rejects hidden or generated targets', async () => {
    const root = await projectRoot()
    try {
      const { host } = fixture()
      host.workspaceRegistry.resolveByPath = vi.fn(async () => ({ path: root, sessionIds: ['session-1'] }))
      ...
      expect(await handler('proposal.prepare', { sessionId: 'session-1', proposal }, signal))
        .toMatchObject({ ok: true, value: { create: { applicable: true, missingDirectories: ['资料'] } } })
      expect(await handler('proposal.apply', { sessionId: 'session-1', proposal, expectedVersions: { [proposal.path]: '' } }, signal))
        .toMatchObject({ ok: true, value: { path: proposal.path, operation: 'create' } })
      expect(await fs.readFile(path.join(root, '资料', '说明.txt'), 'utf8')).toBe('港口见闻\n')

      const hidden = await handler('proposal.prepare', {
        sessionId: 'session-1',
        proposal: { ...proposal, path: '.dsh-editor/secret.md' },
      }, signal)
      expect(hidden).toMatchObject({ ok: false, error: { code: 'bad-request' } })
      const splitTarget = await host.fs.stat(await host.fs.resolve('资料/说明.txt', { cwd: root }))
      const splitTxt = await handler('proposal.prepare', {
        sessionId: 'session-1',
        proposal: {
          marker: 'dsh-editor.proposal',
          version: 2,
          ...
          anchor: '港口',
          newPath: '资料/续.md',
          targetVersion: String(splitTarget!.version),
        },
      }, signal)
      expect(splitTxt).toMatchObject({ ok: true, value: { split: { kind: 'split' } } })
      const generated = await handler('proposal.prepare', {
        sessionId: 'session-1',
        proposal: { ...proposal, path: 'dist/out.md' },
      }, signal)
      expect(generated).toMatchObject({ ok: false, error: { code: 'bad-request' } })
    } finally {
      await fs.rm(root, { recursive: true, force: true })
    }
  })

  it('applies a V2 rename that moves and renames a TXT file between ordinary directories', async () => {
    const root = await projectRoot()
    try {
      await fs.mkdir(path.join(root, 'notes', 'nested'), { recursive: true })
      await fs.mkdir(path.join(root, 'drafts'), { recursive: true })
      await fs.writeFile(path.join(root, 'notes', 'nested', 'a.txt'), 'nested', 'utf8')
      const { host } = fixture()
      host.workspaceRegistry.resolveByPath = vi.fn(async () => ({ path: root, sessionIds: ['session-1'] }))
      host.fs = new NodeFileSystem(root) as unknown as FileSystemLike
      const handler = rpcHandler(host)
      const signal = new AbortController().signal
      const fromVersion = await host.fs.stat(await host.fs.resolve('notes/nested/a.txt', { cwd: root }))
      const proposal = {
        marker: 'dsh-editor.proposal',
        version: 2,
        kind: 'renames',
        summary: '改名并移动',
        renames: [{ from: 'notes/nested/a.txt', to: 'drafts/renamed.txt', version: String(fromVersion!.version) }],
      }
      const prepared = await handler('proposal.prepare', { sessionId: 'session-1', proposal }, signal) as {
        ok: true
        value: { renames: { versions: Record<string, string> } }
      }
      expect(prepared.ok).toBe(true)
      const applied = await handler('proposal.apply', {
        sessionId: 'session-1',
        proposal,
        expectedVersions: prepared.value.renames.versions,
      }, signal) as { ok: true; value: { applied: string[] } }
      expect(applied).toMatchObject({ ok: true, value: { applied: ['drafts/renamed.txt'] } })
      expect(await fs.readFile(path.join(root, 'drafts', 'renamed.txt'), 'utf8')).toBe('nested')
      await expect(fs.stat(path.join(root, 'notes', 'nested', 'a.txt'))).rejects.toMatchObject({ code: 'ENOENT' })
    } finally {
      await fs.rm(root, { recursive: true, force: true })
    }
  })

  it('rejects a V2 split whose generation baseline is stale before prepare', async () => {
    const root = await projectRoot()
    try {
      await fs.writeFile(path.join(root, '正文', '001.md'), '前\n## 第二幕\n后', 'utf8')
      const { host } = fixture()
      host.workspaceRegistry.resolveByPath = vi.fn(async () => ({ path: root, sessionIds: ['session-1'] }))
      host.fs = new NodeFileSystem(root) as unknown as FileSystemLike
      const handler = rpcHandler(host)
      const generation = await host.fs.stat(await host.fs.resolve('正文/001.md', { cwd: root }))
      await fs.writeFile(path.join(root, '正文', '001.md'), '前改了\n## 第二幕\n后', 'utf8')
      const result = await handler('proposal.prepare', {
        sessionId: 'session-1',
        proposal: {
          marker: 'dsh-editor.proposal',
          version: 2,
          ...
          anchor: '## 第二幕',
          newPath: '正文/002.md',
          targetVersion: String(generation!.version),
        },
      }, new AbortController().signal)
      expect(result).toMatchObject({ ok: false, error: { details: { reason: 'STALE' } } })
    } finally {
      await fs.rm(root, { recursive: true, force: true })
    }
  })

  it('rechecks V2 basis inside the existing workspace write lock on apply', async () => {
    const root = await projectRoot()
    try {
      await fs.writeFile(path.join(root, '正文', '001.md'), '# 第一章\n', 'utf8')
      const { host } = fixture()
      ...
      const lockKey = (await host.fs.resolve('.', { cwd: root })).targetKey
      let release!: () => void
      const held = new Promise<void>((resolve) => { release = resolve })
      const blocker = withWorkspaceWrite(lockKey, async () => { await held })
      const applyPromise = handler('proposal.apply', {
        sessionId: 'session-1',
        proposal,
        expectedVersions: { [proposal.path]: '' },
      }, new AbortController().signal)
      await new Promise((resolve) => setTimeout(resolve, 30))
      await fs.writeFile(path.join(root, '正文', '001.md'), '# 第一章\n已改\n', 'utf8')
      release()
      await blocker
      const applied = await applyPromise as { ok: boolean; error?: { details?: { reason?: string } } }
      expect(applied.ok).toBe(false)
      expect(applied.error?.details?.reason).toBe('STALE')
      await expect(fs.stat(path.join(root, '世界书', '港口.md'))).rejects.toMatchObject({ code: 'ENOENT' })
    } finally {
      await fs.rm(root, { recursive: true, force: true })
    }
  })

  it('keeps apply basis and target checks inside the existing workspace write closures', async () => {
    const workbench = await fs.readFile(new URL('./index.ts', import.meta.url), 'utf8')
    const rpc = await fs.readFile(new URL('./rpc/proposal.ts', import.meta.url), 'utf8')
    const manuscript = await fs.readFile(new URL('../../dsh-manuscript/src/index.ts', import.meta.url), 'utf8')
    const applySource = await fs.readFile(new URL('../../dsh-manuscript/src/rpc/proposal.ts', import.meta.url), 'utf8')
    expect(workbench).toContain('handler.mutation ? withWorkspaceWrite(access.root.targetKey, run)')
    expect(rpc).toMatch(/'proposal\.apply':\s*\{\s*mutation:\s*true/)
    expect(rpc).toContain('await assertWritingProposalBasis(files, proposal.basis)')
    expect(rpc).toContain('targetVersion: proposal.targetVersion')
    expect(manuscript).toContain("'proposal.apply'")
    expect(manuscript).toContain('withWorkspaceWrite(access.root.targetKey, run)')
    expect(applySource).toContain('await assertWritingProposalBasis(context, proposal.basis)')
