 ...
 *
 * The token block comes from dsh-editor-seats/tokens (single source,
 * canonical table: docs/ui.md) and is inlined at build time. The shell
 * ships the same string via `redesignedStyles`, so when both run together
 * the injected rules are byte-identical and order cannot matter.
