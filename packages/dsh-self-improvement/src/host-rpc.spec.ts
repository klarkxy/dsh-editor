...
async function host(rejection?: number, omitPolicy = false) {
  ...
  const call = vi.fn(async () => ({ ok: true as const, value: { configured: false } }))
  registerHostRpc({ connection: omitPolicy ? {} : { requestRejection: () => rejection }, webServer: {
    register: route => { handler = route.handler; return () => {} },
  } }, '/dsh-self-improvement', call)
  const server = createServer((req, res) => { void handler(req, res) })
  servers.push(server); server.listen(0, '127.0.0.1'); await once(server, 'listening')
  const address = server.address()
  if (!address || typeof address === 'string') throw new Error('No server address')
  return { call, url: `http://127.0.0.1:${address.port}/dsh-self-improvement/status` }
}
...
it('fails closed when the host authorization policy is missing', async () => {
  ...
  expect((await request(url)).status).toBe(503); expect(call).not.toHaveBeenCalled()
})
it('uses the existing client-request envelope and disables caching', async () => {
  const { call, url } = await host()
  const response = await request(url)
  expect(response.status).toBe(200); expect(response.headers.get('cache-control')).toBe('no-store')
  expect(await response.json()).toEqual({ type: 'server-response', rpcId: 'test', result: { ok: true, value: { configured: false } } })
  expect(call).toHaveBeenCalledOnce()
})
it('rejects invalid method envelopes before dispatch', async () => {
  const { call, url } = await host()
  expect((await request(url, { type: 'client-request', rpcId: 'test', method: 'extract', payload: {} })).status).toBe(400)
  expect(call).not.toHaveBeenCalled()
})
