...
Default-enabled self-improvement: candidate lessons in shared Memory, review, and optional Markdown skill export.

[简体中文](https://github.com/klarkxy/dsh-editor/blob/main/packages/dsh-self-improvement/docs/README.zh-CN.md)

## Install

Requires Node.js ≥22 and DSH `0.1.5-rc.2`. The bundle entry starts enabled and remains independently switchable. It does **not** load Memory or turn on Dream; standalone hosts must install and load `@klarkxy/dsh-memory` separately.
...
Open **Settings → 自我改进** to review candidates and skill drafts. The plugin does not render status, empty states, or management controls in chat.

## Behaviour

Candidates come only from explicit human corrections or a tool failure that is later verified by a successful result for the same tool. Silence and model self-report are not treated as success. Candidates are stored as Memory `kind: lesson` with `source: self-improvement` and do not enter prompts until accepted. Host pre-step injection uses `createUserMessage`, keeps decision flags such as `startsRequestSeries`, and only inserts active unexpired lessons relevant to the current user request (at most 5, ~800 tokens including the wrapper). Disabling this plugin or Memory mid-await stops lesson injection even if Memory stays on.
