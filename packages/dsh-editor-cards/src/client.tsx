...
import { CardsDetailSeat } from './client/detail.tsx'
import { CardsPanelSeat, type RpcCaller } from './client/panel.tsx'
...
export function apply(ctx: Context): void {
  ...
  const renderPanel = (props: unknown) => {
    ...
    if (!seat) return null
    return <CardsPanelSeat {...seat} rpc={client.connection.rpc} />;
  }
  const renderDetail = (props: unknown) => {
    const seat = seatFromRenderProps(props)
    if (!seat) return null
    return <CardsDetailSeat {...seat} rpc={client.connection.rpc} />;
  }
