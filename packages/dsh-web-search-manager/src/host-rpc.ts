...
export function registerHostRpc(ctx: HostRpcContext, channel: string,
  call: (endpoint: string, payload: unknown, signal: AbortSignal) => Promise<RpcResult>): () => void {
  return ctx.webServer.register({ kind: 'prefix', path: channel, handler: async (req, res) => {
    ...
    try {
      ...
      const result = await call(endpoint, request.payload, controller.signal)
      if (!res.destroyed) {
        res.writeHead(200, { 'content-type': 'application/json', 'cache-control': 'no-store' })
        res.end(JSON.stringify({ type: 'server-response', rpcId: request.rpcId, result }))
      }
    } catch {
      if (!res.destroyed) { res.writeHead(500); res.end('network settings request failed') }
    }
  } })
}
