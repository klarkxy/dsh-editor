...
describe('editor proposal boundary', () => {
  ...
  it('allows only Markdown search/read/propose tools', () => {
    ...
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'merge', path: 'a.md', sourcePath: 'notes/nested/b.txt', summary: 'x' } })).toBeUndefined()
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'renames', summary: 'x', renames: [{ from: 'notes/a.txt', to: 'drafts/renamed.txt' }] } })).toBeUndefined()
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'split', path: 'notes/a.md', newPath: '../evil.md', summary: 'x' } })).toContain('Only project-relative')
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'split', path: 'dist/a.txt', newPath: 'notes/b.md', summary: 'x' } })).toContain('Only project-relative')
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'edit', path: '.dsh-editor/作品索引.md', summary: 'x' } })).toContain('Only project-relative')
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'create', path: 'notes/a.json', summary: 'x' } })).toContain('Only project-relative')
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'chapter_plan', path: '正文/001.md', summary: 'x' } })).toContain('大纲/')
    expect(editorToolGuard({ name: 'writing_propose', arguments: { kind: 'explode', path: 'notes/a.md', summary: 'x' } })).toBeDefined()
