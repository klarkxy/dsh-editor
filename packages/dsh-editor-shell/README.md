...
桌面写作 shell 调用 `dsh-editor-workbench` 拥有的 loopback-only `/dsh-editor-workbench` RPC 创建和列出作品文本快照，并以“恢复为新副本”把选定快照恢复到用户选择的空目录。普通稿件读写、搜索与 AI 建议仍走公开 `/manuscript`；界面不会直接访问 Node 文件系统。未保存的编辑 buffer 不属于快照内容，原作品不会被恢复操作改写。
...
connection contracts. Its Host entry is intentionally minimal; project lifecycle
and novel tools belong to the two private Host plugins. It is not a public
installable plugin.
