...
function ImportDialog(props: { flow: ImportFlow; onCancel(): void; onApply(): void; onContinue(): void; onCleanup(): void }) {
  ...
  return (
    <Dialog
      ...
      title={title}
      className="file-dialog import-dialog file-dialog-overlay import-overlay"
      overlayClassName="file-dialog-overlay import-overlay"
      dismissible={!working}
      initialFocusRef={focus}>
      <Flex direction="column" gap="3">
        <Heading as="h2" size="4" id="import-dialog-title">
          {title}
        </Heading>
        {working ? <Flex className="import-working" align="center" gap="3">
          ...
          {t('import.cleanupBody')}
            ...
            : recover ? <Flex direction="column" gap="2">
          <Text size="2">
            {cleaning ? t('import.cleanupOnly') : t('import.recoverBody', { summary: importSummary(probe!) })}
          </Text>
          {probe?.message && !cleaning ? <Callout.Root className="warning" color="red" role="alert" size="1">
            <Callout.Text>
              {probe.message}
            </Callout.Text>
          </Callout.Root> : null}
        </Flex>
              : probe ? <Flex direction="column" gap="2">
          <Text size="2">
            {t('import.reviewBody', { summary: importSummary(probe) })}
          </Text>
          {probe.skipped.length ? <Text size="2">
        ...
        {!working ? <Flex justify="end" gap="2" wrap="wrap">
