...
import { AUTHOR_PREFERENCES_KEY } from './author-preferences.ts'
import { COMPLETION_PREFERENCE_KEY } from './completion-preference.ts'
import { DEFAULT_WRITING_PREFERENCES, migrateLegacyWritingPreferences, writingPreferences, type WritingPreferences } from './writing-settings.ts'
...
describe('writing preference migration', () => {
  ...
  it('lets an existing DSH user field win and removes only its matching legacy key', async () => {
    ...
    const scope = scopeWith({
      status: 'ready', value: { completion: 'manual', authorPreferences: '' }, user: { completion: 'manual' }, base: {}, revision: 1, writable: true, mode: 'host',
    }, async (...args) => { set(...args) })
