...
Background recap generation and separate agent checkpoints. The plugin starts enabled and turns **回顾卡片**, agent checkpoints, and semantic checkpoints on by default so sessions stay coherent without per-author toggles; each can still be switched off. Recap is display-only and is not model context.

Requires Node.js ≥22 and DSH `0.1.7-alpha.1`. Auxiliary generation uses `@klarkxy/dsh-ai-services` when that service is present. `@deepseek-ai/dsh-llm` and `@deepseek-ai/dsh-session` are required peers.
