...
Background recap generation and separate agent checkpoints. The plugin starts enabled. Background recaps, agent checkpoints, and semantic checkpoints are each on by default and can be switched off independently. Recaps are display-only and are not model context.
